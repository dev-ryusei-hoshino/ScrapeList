/*
 * AlightMotion Premium Generator V2
 *
 * Author: Ryusei Hoshino (https://github.com/dev-ryusei-hoshino)
 * Base: https://amprem.sparxiee.web.id/
...
async function sendLink(email) {
  const res = await fetch("https://amprem.sparxiee.web.id/api/am/send", {
    method: "POST",
    headers: {
...
async function verifyLink(email, link) {
  const res = await fetch("https://amprem.sparxiee.web.id/api/am/verify", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Accept: "application/json",
